...
| removeEventListener('click', fn)   | 解绑事件，因为监听事件可能会有多个回调函数，所以解绑事件时函数名必须是绑定的函数的其中一个，也就是说绑定时的回调函数一定要写成外部函数，供解绑时引用 |
...
test()  可用来判断是否匹配，`/前端/.test('前端开发')` 返回true；exec() 类似，但返回的是一个数组
