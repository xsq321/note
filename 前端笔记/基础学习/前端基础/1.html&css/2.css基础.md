...
| div:hover, a:active, p:nth-child(2){} | 伪类选择器：悬浮时的div的样式；a标签鼠标按下去时的样式；选择子元素，nth-child(-n+5) 表示取前五个，可以用各种公式进行选择，nth-child是从第1个开始选的，没有第0个； |
...
| visibility: visible, hidden, collapse                        | hidden 表示元素不可见（不绘制），但仍会占用空间，可防止回流  |
...
| position: relative, absolute, fixed, sticky                  | 子绝父相，且可以将行内元素转换成脱离文档流的块级元素；fixed 比较特殊是相对于浏览器窗口进行定位的，可用于侧边导航及==视频的网页全屏== ，但不能有任何祖先元素设置了 transform、perspective 或 filter 样式属性 |
| cursor                                                       | pointer,  text,                                              |
