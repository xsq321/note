...
用于在函数组件中进行状态管理，接收一个初始状态，并返回一个数组，其中包含当前状态和一个函数，函数可用于更新当前状态
...
export default function Test() {
  ...
  return (
    <div>
      <div>{state.num}</div>
      // 外部只需调用 dispatch 并传入想实现的操作类型即可，不需要知道具体的实现
...
const newVal = produce(arr, draft => {
...
除了 useCallback 中的过期闭包外，在 useEffect 中的异步函数以及 setState() 的异步调用都有可能产生过期闭包问题，其中 setState() 可采用 setState((a) => a + 1) 这种方式修改值，具体查看 详细笔记-5-31 
