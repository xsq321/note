...
navigationBarTitleText ：可以设置每个页面自己的标题
...
addCount(event){  // 默认会传递 event 参数，也只能传递该参数
  ...
  this.setData({ count: this.data.count + val }) // 变量的修改，通过 this 访问
},
```
...
就是创建页面时 .js 中的那些生命周期函数，如 onLoad, onUnLoad
...
没有跨域限制，因为根本就不是浏览器；但是对于请求的域名需要在小程序后台中进行配置：开发管理 - 开发设置 - 服务器域名
...
| image       | 图片，`<image src="url" mode="widthFix" show-menu-by-longpress="{{true}}" />` |
| swiper      | 轮播图                                                       |
| scroll-view | 可滚动视图区域；可监听一些事件，如滚动到底部等               |
| icon        | 图标，`<icon color="red" type="success" size="30"></icon>`   |
| checkbox    | 多选框；点击时自动触发 checkbox-group 的 change 事件，当然也可为每个 checkbox 绑定 tap 点击事件 |
| ...         |                                                              |
